...
class ModelTraining:
    ...
    def train_lgbm(self, X_train, y_train):
        try:
            ...
            logger.info("Starting hyperparameter tuning with RandomizedSearchCV")    # Using RandomizedSearchCV for hyperparameter tuning
            random_search.fit(X_train, y_train)
            logger.info("Hyperparameter tuning completed")

            best_params = random_search.best_params_         
            best_lgbm_model = random_search.best_estimator_
            logger.info("Best parameters found are: {}".format(best_params))
    ...
    def evaluate_model(self, model, X_test, y_test):
        try:
            logger.info("Evaluating our LGBM model")
            ...
            return {   # We have to return a dictionary of metrics
                'accuracy': accuracy,
                'precision': precision,
                'recall': recall,
                'f1_score': f1
    ...
    def save_model(self, model):
        try:
            ...
            logger.info("Model saved to {}".format(self.model_output_path))
        except Exception as e:
